...
export default class Time {
    ...
    // 23 | 23:03 | 23:03:04
    getTimeObj(time = '') { // eslint-disable-line complexity
        const t = `${time}`
        let split = t.split(':')
        if (!split || split.length === 1)
            split = t.split('.')
    ...
    getDateTime(date = new Date()) {
        ...
        if (split.length === 1) {
            if (split.indexOf(':') > -1 || split.indexOf('-') > -1) {
                const timeObj = this.getTimeObj(split[0])
                if (timeObj) {
            ...
            const dateFromString = this.getDate(split[0])
            const timeObj = this.getTimeObj(split[1])
            if (timeObj)
                res = this.setDateTime(timeObj, dateFromString)
            else
                res = dateFromString
        }
        return res
    ...
    getMonthStartDate(date = new Date()) {
    ...
    getWeekday(day) { // eslint-disable-line complexity
